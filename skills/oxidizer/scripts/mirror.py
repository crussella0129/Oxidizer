...
# worked examples
#
# Opt-in, and deliberately not enabled by default. Everything else Oxidizer
# mirrors is canon — normative, or generated by the compiler. A community
# algorithms repository is neither, and folding it in silently would undercut
# the precedence hierarchy the whole skill rests on. One flag away, clearly
# labelled, lowest precedence.

ALGORITHMS_DEFAULT_URL = "https://github.com/TheAlgorithms/Rust.git"

# Files that are re-export plumbing rather than an implementation.
_SKIP_RUST = re.compile(r"(?:^|/)(?:mod|lib|main)\.rs$")

# Default-warn style lints a corpus might opt out of. Recorded so the skill can
# say what posture the examples were written under instead of assuming ours.
_NOTABLE_ALLOWS = {
    "needless_range_loop", "needless_return", "ptr_arg", "len_zero",
    "unwrap_used", "expect_used", "indexing_slicing", "panic",
    "single_match", "redundant_closure", "manual_memcpy",
}


def _declared_lint_allows(repo: Path) -> list[str]:
    """Style lints the example repo turns off in its own Cargo.toml.

    Parsed rather than measured: getting per-file lint status would require
    building the repository and its dependencies, which is far more than a
    corpus build should cost. `oxidize diagnose <repo> --clippy` remains
    available for anyone who wants the precise answer.
    """
    ...
    try:
        ...
        return []
    section = re.search(r"^\[lints\.clippy\](.*?)(?=^\[|\Z)", text, re.M | re.S)
    if not section:
        return []
    allowed = set()
    for m in re.finditer(r'^\s*([A-Za-z_][A-Za-z_0-9]*)\s*=\s*(.+)$',
                         section.group(1), re.M):
        name, value = m.group(1), m.group(2)
        if '"allow"' in value or "'allow'" in value:
            allowed.add(name)
    return sorted(allowed & _NOTABLE_ALLOWS)


def _ensure_algorithms_checkout(spec: str, corpus: Path, quiet: bool) -> Path | None:
    candidate = Path(spec).expanduser()
    if candidate.is_dir():
        return candidate
    if not spec.startswith(("http://", "https://", "git@")):
        print(f"error: --algorithms path not found: {candidate}", file=sys.stderr)
        return None

    dest = corpus / ".src" / "algorithms"
    if (dest / ".git").is_dir():
        if not quiet:
            print(f"  reusing checkout at {dest}", file=sys.stderr)
        return dest
    dest.parent.mkdir(parents=True, exist_ok=True)
    if not quiet:
        print(f"  cloning {spec} -> {dest}", file=sys.stderr)
    try:
        subprocess.run(["git", "clone", "--depth", "1", spec, str(dest)],
                       capture_output=True, text=True, check=True, timeout=600)
    except (subprocess.CalledProcessError, subprocess.TimeoutExpired,
            FileNotFoundError) as exc:
        detail = getattr(exc, "stderr", "") or str(exc)
        print(f"  algorithms  SKIPPED (clone failed: {detail.strip()[:160]})",
              file=sys.stderr)
        return None
    return dest


def _public_repo_url(spec: str, repo: Path) -> str:
    """Best public browse URL for the checkout, or "" if there isn't one.

    `git remote get-url` is not trustworthy here: behind a git proxy the origin
    comes back as something like `http://local_proxy@127.0.0.1:41729/git/owner/repo`,
    and citing that would hand the user a link only this machine can open. The
    spec wins when it is itself a URL, and anything pointing at loopback or
    carrying credentials is rejected rather than guessed at.
    """
    candidates = [spec]
    try:
    ...
    for candidate in candidates:
        if not candidate:
            continue
        url = candidate.replace("git@github.com:", "https://github.com/")
        ...
        if not host or host in ("localhost", "127.0.0.1", "::1") \
            ...
            continue
        # Drop any embedded credentials before the URL is written into docs.
        clean = f"https://{host}{parsed.path}"
        return re.sub(r"\.git$", "", clean).rstrip("/")
    return ""


def build_algorithms_source(spec: str, corpus: Path, quiet: bool) -> dict | None:
    repo = _ensure_algorithms_checkout(spec, corpus, quiet)
    if repo is None:
        return None
    ...
    for root in roots:
        files.extend(
            ...
            if not _SKIP_RUST.search(str(p.relative_to(root)).replace("\\", "/"))
        )
    if not files:
        print("  algorithms  SKIPPED (no .rs files found)", file=sys.stderr)
        return None

    out_dir = corpus / "algorithms"
    if out_dir.exists():
        shutil.rmtree(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    # Cite the exact commit: worked examples are not versioned like the canon,
    # so the revision is the only thing that makes a citation reproducible.
    rev = ""
    try:
        ...
        pass
    web = _public_repo_url(spec, repo)

    entries: list[dict] = []
    for path in files:
        root = next(r for r in roots if path.is_relative_to(r))
        ...
        category = doc_id.rsplit("/", 1)[0] if "/" in doc_id else "general"
        name = doc_id.rsplit("/", 1)[-1]
        title = f"{category.replace('/', '::')}::{name}"
        md, headings, items = rust_source_to_markdown(source, title)

        url = (f"{web}/blob/{rev or 'master'}/{root.name}/{rel}" if web
               else f"(local checkout @ {rev or 'unknown rev'}) {root.name}/{rel}")
        out_file = out_dir / (doc_id + ".md")
        out_file.parent.mkdir(parents=True, exist_ok=True)
        out_file.write_text(
            f"<!-- oxidizer: source=algorithms url={url} -->\n\n" + md,
            encoding="utf-8")

        public = [i["name"] for i in items if i["kind"] != "impl" and i["name"]]
        summary = ""
        for item in items:
            if item["doc"]:
                summary = item["doc"].splitlines()[0]
                break
        if not summary:
            summary = f"{name.replace('_', ' ')} implementation in {category}."

        entries.append({
            "id": doc_id, "source": "algorithms", "kind": "example",
    ...
    entries.sort(key=lambda e: e["id"])
    (out_dir / "INDEX.json").write_text(
        json.dumps({"source": "algorithms", "docs": entries}, indent=1),
        encoding="utf-8")

    allows = _declared_lint_allows(repo)
    if not quiet:
        print(f"  {'algorithms':16s} {len(entries):5d} docs  "
              f"(worked examples @ {rev or 'unknown rev'})", file=sys.stderr)
        if allows:
            print(f"    note: this corpus allows {', '.join(allows)} — not an "
                  f"idiom authority", file=sys.stderr)

    role = ("Worked implementations of algorithms and data structures. "
            "NOT CANON: community code, illustrative only, lowest precedence. "
            "Use for structure, never to settle what is idiomatic or correct.")
    if allows:
        role += f" Its own config allows: {', '.join(allows)}."
    return {
        "id": "algorithms", "kind": "examples",
        "title": "Worked Rust examples (community algorithms corpus)",
        "role": role, "url": web or ALGORITHMS_DEFAULT_URL, "origin": "community",
        "revision": rev, "canonical": False, "lint_allows": allows,
...
def main() -> int:
    ...
    ap.add_argument("--algorithms", nargs="?", const=ALGORITHMS_DEFAULT_URL,
                    default=None, metavar="PATH_OR_URL",
                    help="also mirror a worked-examples corpus of Rust source "
                         "(default: TheAlgorithms/Rust). Not canon — indexed at "
                         "lowest precedence and labelled as illustrative.")
    ...
    if args.algorithms:
        if not args.quiet:
            print("\nWorked examples (non-canonical):", file=sys.stderr)
        info = build_algorithms_source(args.algorithms, corpus, args.quiet)
        if info:
            built.append(info)
